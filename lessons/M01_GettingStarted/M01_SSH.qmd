...
1. Get to the command line (i.e. the shell).
    1. On a Mac, open Terminal.
    2. If you are on Windows and you have admin rights, first install `git-bash`. Otherwise try to access the Linux subsystem.
...
2. Move into your root directory and enter `cd`.
3. Generate the key.
...
6.  
    ...
    If you\'re using macOS Sierra 10.12.2 or later, you will need to
    modify your `~/.ssh/config` file to automatically load keys into the
    ssh-agent and store passphrases in your keychain. Follow the
    instructions
    [here](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent?platform=mac#adding-your-ssh-key-to-the-ssh-agent)
    (at step 2).
...
3.  
    ...
    The key should begin `ssh-ed25519` and end with the email address
...
On GitHub, go to your account settings and select \"SSH and GPG Keys\"
from the side menu.
...
1.  
    ...
    Add a brief title describing the context of the key, i.e. the local
