...
Although in many ways superceded the `plyr` and `dplyr`, [they remain useful](https://stackoverflow.com/questions/3505701/grouping-functions-tapply-by-aggregate-and-the-apply-family).
:::
...
Certainly!
The `arrange()` function in `dplyr` is used to order the rows of a dataset based on values in one or more columns.
While `arrange()` itself is relatively straightforward, there are a few helper functions that can be useful in conjunction with it.
